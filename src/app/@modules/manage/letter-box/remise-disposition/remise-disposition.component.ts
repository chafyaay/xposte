...
export class RemiseDispositionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.TEMP_FIELD_LISTS.message_lus_list = [
      ...this.FIELD_LISTS.message_lus_list,
    ];
  ...
  REMOVE_FIELD(field: any) {
    if (this.selectedListType === 'message_lus') {
      ...
      if (
        this.TEMP_FIELD_LISTS.message_lus_list.every(
      ...
      if (
        this.TEMP_FIELD_LISTS.sujet_list.every(
          (item) => item.disabled === false
        )
      ) {
        this.isFormValid = false;
  ...
  onKeyUp(event: any, fcn: any) {
    ...
    if (fcn === 'codeCompostage') this.getElemById(fcn).maxLength = 5;
    else if (fcn === 'version') this.getElemById(fcn).maxLength = 6;
  ...
  setValidator(fcn: any, value?: any) {
    if (value) this.form.get(fcn).patchValue(value);
    ...
    if (fcn) {
      if (this.form.get(fcn).valid) {
        if (fcn === 'fichiersCompresses' || fcn === 'fichiersChiffres') {
          if (this.form.get(fcn).value === 1) this.REMISE_DISPO_OBJ[fcn] = true;
          else if (this.form.get(fcn).value === 2)
            this.REMISE_DISPO_OBJ[fcn] = false;
          else this.REMISE_DISPO_OBJ[fcn] = 'null';
        }
        ...
        this.REMISE_DISPO_EVENT.emit({ isFormValid: this.is_form_valid() });
