...
import { remiseDispoI, SUBFIELDSI } from '__src/app/@shared/models/remise-dispo';
...
export class RemiseDispositionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.FIELD_LISTS={
      list1: [
        { id: 1, label: FIELD.EMMETTEUR, fcn: 'emetteur', visible: false, type: 'text' },
        { id: 2, label: FIELD.DESITINATAIRE, fcn: 'destinataire', visible: false, type: 'text' },
        { id: 3, label: FIELD.CONTENT_DESC, fcn: 'contentDescription', visible: false, type: 'text' },
        { id: 4, label: FIELD.FICHIER_COMPRESSEE, fcn: 'fichiersCompresses', visible: false, type: 'list', data: 
        [
        ...
        { id: 5, label: FIELD.FICHIER_COMPRESSEE,fcn: 'fichiersChiffres', visible: false, type: 'list', data: [
          {id:0,label:FIELD.INDIFFERENT},
          {id:1,label:FIELD.OUI},
          {id:2,label:FIELD.NON}
        ] },
      ...
      list2:   [
        { id: 1, label: FIELD.TYPE, fcn: 'type', visible: false, type: 'text' },
        { id: 2, label: FIELD.VERSION,fcn: 'version', visible: false, type: 'text' },
        { id: 3, label: FIELD.CODE_EMETTEUR, fcn: 'codeEmetteur', visible: false, type: 'text' },
        { id: 4, label: FIELD.CODE_COMPOSTAGE,fcn: 'codeCompostage', visible: false, type: 'text' },
        { id: 5, label: FIELD.DATE_HEURE_SUJET, fcn: 'dateHeureSujet', visible: false, type: 'date' },
        { id: 6, label: FIELD.NBFSE, fcn: 'nbFSE', visible: false, type: 'text' },
        { id: 7, label: FIELD.SUJET_OUT_REPLY_INFO, fcn: 'sujetOuReplyInTo', visible: false, type: 'text' },
      ]
    }
    ...
    console.clear()
    console.log("this.translate.translations['fr-FR']")
    const currentLang=this.translate.currentLang;
    console.log(currentLang)
    console.log(this.translate.translations[currentLang].criters.EMMETTEUR)
...
@Output() REMISE_DISPO_EVENT:EventEmitter<any>=new EventEmitter();
 ...
 /*    if(this.f.valid){
      obj[fcn]=this.f.get(fcn).value;
      this.remiseDispoObj={...obj};
      this.rmiseDispoDataEvent.emit({ isvalid: this.f.valid, ...this.remiseDispoObj });
    }else {
      this.rmiseDispoValidFormEvent.emit(this.f.valid)
    } */
  }
 ...
 // insert filter field 
  insertField(listType: string) {
    ...
    this.FIELD_LISTS[listType].map((item: any) => {
      if (item.id === this.selectedFieldId) item.visible = true;
    });
  }
 // remove filter field 
  removeField(fcn:any,listType:string) { 
    this.FIELD_LISTS[listType].map((item: any) => {
      if (item.fcn === fcn) item.visible = false;
    });
