...
selectedForm=""
  validateForm(form?: any) {
    console.clear()
    console.log(form)
    const keys = Object.keys(form).map(item=>item.toLocaleLowerCase());
    console.log(keys)
    if (keys.includes('emetteur') && keys.includes('destinataire')) {
      this.selectedForm='messageLus'
      ...
      this.selectedForm='sujet'
      this.sujetForm = form;
      this.isSujetFormValid = form.isValid;
      this.isSujetFormEmpty = form.isEmpty;
    }

    else if (keys.includes('dateheuredebut') && keys.includes('dateheurefin')) {
      this.periodeValidator();
      this.selectedForm='periode'
    ...
    if((!this.isMessageLusFormEmpty && !this.isSujetFormEmpty)){
      ...
      console.log("--- 2")
console.log(form)
console.log(this.isMessageLusFormEmpty +"&&"+ this.isSujetFormEmpty)
